...
; 0000 003E DDRD=(0<<DDD7) | (0<<DDD6) | (0<<DDD5) | (0<<DDD4) | (0<<DDD3) | (0<<DDD2) | (0<<DDD1) | (0<<DDD0);
	OUT  0x11,R30
; 0000 003F // State: Bit7=T Bit6=T Bit5=T Bit4=T Bit3=T Bit2=P Bit1=P Bit0=P
; 0000 0040 PORTD=(0<<PORTD7) | (0<<PORTD6) | (0<<PORTD5) | (0<<PORTD4) | (0<<PORTD3) | (1<<PORTD2) | (1<<PORTD1) | (1<<PORTD0);
	LDI  R30,LOW(7)
	OUT  0x12,R30
; 0000 0041 
...
; 0000 0092 if(semaforo==1){
	SBRS R2,2
	RJMP _0x6
; 0000 0093 dec_seg++; //Se incrementa la variable de las decimas de segundo
	INC  R5
; 0000 0094 
; 0000 0095 if (dec_seg==10){ // Evalua si se debe aumentar las decimas
	LDI  R30,LOW(10)
	CP   R30,R5
	BRNE _0x7
; 0000 0096 dec_seg=0; // Reestablece las decimas de segundo a cero
	CLR  R5
; 0000 0097 seg1++; // Aumenta las unidades de segundo
	INC  R4
; 0000 0098 
; 0000 0099 if(seg1==10){  //  Evalua si se ha llegado al limite de las unidades de segundo
	CP   R30,R4
	BRNE _0x8
; 0000 009A seg1=0; // Reestablece las unidades de segundo a cero
	CLR  R4
; 0000 009B seg2++; // Aumenta las decenas de segundo
	INC  R7
; 0000 009C 
; 0000 009D if(seg2==10){
	CP   R30,R7
	BRNE _0x9
; 0000 009E seg2=0;
	CLR  R7
; 0000 009F semaforo=0;
	CLT
	BLD  R2,2
; 0000 00A0 }
; 0000 00A1 
...
_0x7:
	...
	LDI  R27,0
	RCALL _delay_ms
; 0000 00A5 } // if semaforo
; 0000 00A6 
; 0000 00A7 if(reset==0){
_0x6:
	SBIC 0x10,0
	RJMP _0xA
; 0000 00A8 dec_seg=0;
...
_0xA:
	SBIC 0x10,1
	RJMP _0xB
; 0000 00AE semaforo=0;
	CLT
	BLD  R2,2
; 0000 00AF 
; 0000 00B0 if(start==0)
_0xB:
	SBIC 0x10,2
	RJMP _0xC
; 0000 00B1 semaforo=1;
	SET
	BLD  R2,2
; 0000 00B2 /*
; 0000 00B3 
; 0000 00B4 if (reset==0)
; 0000 00B5 botona=0;
; 0000 00B6 else
; 0000 00B7 botona=1;
; 0000 00B8 if ((botonp==1)&&(botona==0)) //hubo cambio de flanco de 1 a 0
; 0000 00B9 {
; 0000 00BA 
; 0000 00BB }
; 0000 00BC delay_ms(40); //Se coloca retardo de 40mS para eliminar rebotes
; 0000 00BD }
; 0000 00BE if ((botonp==0)&&(botona==1)) //hubo cambio de flanco de 0 a 1
; 0000 00BF */
; 0000 00C0 delay_ms(40); //Se coloca retardo de 40mS para eliminar rebotes
_0xC:
	LDI  R26,LOW(40)
	LDI  R27,0
	RCALL _delay_ms
; 0000 00C1 
; 0000 00C2 PORTB=tabla7segmentos [seg2]; // Muestra las decenas de segundo
	MOV  R30,R7
	RCALL SUBOPT_0x0
	OUT  0x18,R0
; 0000 00C3 PORTA=tabla7segmentos [seg1]; // Muestra las unidades de segundo
	MOV  R30,R4
	RCALL SUBOPT_0x0
	OUT  0x1B,R0
; 0000 00C4 PORTC=tabla7segmentos [dec_seg]; // Muestra las decimas de segundo
	MOV  R30,R5
	RCALL SUBOPT_0x0
	OUT  0x15,R0
; 0000 00C5 }
